...
async function send(action,payload){
  try{const result=await window.callAmplenotePlugin(action,{noteUUID:state.noteUUID,...payload});if(!result?.ok)throw Error(result?.message || "Could not open link.");}
  catch(error){document.getElementById("preview-error").textContent=error?.message || String(error);}
}
source.addEventListener("click",event=>{event.preventDefault();send("openLink",{url:source.href});});
