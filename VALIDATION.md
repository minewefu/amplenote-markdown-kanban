...
- 52 local tests pass on Node.js 24 on Windows.
...
The first interface run found that Amplenote's iframe disallows native form submission and JavaScript modal dialogs. The current source uses direct buttons and inline confirmation instead. Live card creation and keyboard movement now work. A transient post-insertion read interrupted automatic placement during the first creation test; the current code retries coherent reads and reports the existing task ID if placement fails, preventing duplicate creation on retry. That latest creation correction needs live retesting.
